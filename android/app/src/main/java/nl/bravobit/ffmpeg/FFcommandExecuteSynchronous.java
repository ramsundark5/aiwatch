...
public class FFcommandExecuteSynchronous {
    ...
    private CommandResult runCommand() {
        ...
        try {
            ...
            LOGGER.d("Running publishing updates method");
            checkAndUpdateProcess();
            return CommandResult.getOutputFromProcess(process);
        } catch (TimeoutException e) {
            LOGGER.e("FFmpeg binary timed out", e);
            return new CommandResult(false, e.getMessage());
        } catch (Exception e) {
            LOGGER.e("Error running FFmpeg binary", e);
        } finally {
            Util.destroyProcess(process);
            LOGGER.i("ffmpeg process destroyed");
        }
        LOGGER.e("ffmpeg returned dummy response.");
        return CommandResult.getDummyFailureResponse();
    }
        ...
        while (!Util.isProcessCompleted(process)) {
            ...
            try {
                ...
                while ((line = reader.readLine()) != null) {
                    output += line + "\n";
                    LOGGER.d("ffmpeg progress "+line);
                }
            } catch (IOException e) {
                LOGGER.e("ffmpeg error "+e.getMessage());
            }
        }
    }
